    ...
    <div class="card-container">
        <div class="card bg-base-100 shadow-xl relative">
            ...
            <div class="card-body text-white bg-black bg-opacity-30 p-6 rounded-lg relative">
                ...
                <p class="text-sm">📅 Data Inizio: {{ new Date(travel.StartDate).toLocaleDateString() }}</p>
                <p class="text-sm">📅 Data Fine: {{ new Date(travel.EndDate).toLocaleDateString() }}</p>
                <br />
                <!-- Bottone "Info" in basso a destra -->
                <button class="btn btn-active btn-ghost absolute bottom-2 right-2">
                    Info
                </button>
            </div>
        </div>
