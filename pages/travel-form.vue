  ...
  <div class="container mx-auto max-w-lg p-6 bg-white shadow-md rounded-lg">
    ...
    <form @submit.prevent="submitForm" class="space-y-4">
      ...
      <div>
        ...
        <input v-model="travelData.Name" type="text" class="input input-bordered w-full" required />
      </div>

      <!-- Data Inizio -->
      <div>
        <label class="block text-sm font-medium">Data Inizio</label>
        <input v-model="travelData.StartDate" type="date" class="input input-bordered w-full" required />
      </div>

      <!-- Data Fine -->
      <div>
        <label class="block text-sm font-medium">Data Fine</label>
        <input v-model="travelData.EndDate" type="date" class="input input-bordered w-full" required />
      </div>
        ...
        <div class="dropdown w-full">
          <label tabindex="0" class="btn btn-outline w-full flex justify-between">
            <div class="flex items-center">
              {{ selectedCountry ? countryEmoji(selectedCountry.Code) + " " + selectedCountry.Name : "Seleziona un paese" }}
            </div>
          ...
          <ul tabindex="0"
            ...
            <li v-for="country in countries" :key="country.Id">
              <button @click="travelData.CountryId = country.Id"
